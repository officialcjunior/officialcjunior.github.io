...
This blog post is a brief summary about the work I did in the summer of 2021 with Rizin on 
...
Rizin currently relies upon manually writing code for adding a new CPU or an IO port. This
implementation is unfit as the vast and growing ecology of hardware components such as
...
supporting editable CPU and platform profiles is the goal of this project. This will also make
adding and maintaining these ports easier with less interaction with Rizin’s core. This
project also aims to add more flexibility in having a way of importing existing hardware
data description documents so that reverse engineering on particular chipsets is easier.
This will also help us to memory map the peripheral accesses and registers to provide a
better reverse engineering experience. This, in turn, could also be extended to benefit Rizin
...
![](https://user-images.githubusercontent.com/29057155/112524410-cbab3180-8dc5-11eb-8ba0-79c9be719735.png)
...
During analysis (`aa`), `rz_arch_profile_add_flag_every_io()` is called, which parses the two hashtables and adds 
the information as flags. For that, two new flagspaces were realized: `registers.mmio` and `registers.extended` 
...
t was just as simple as getting the data in the right format, putting it on the corresponding directory and adding it to its `meson.build` :)
...
all (For example, while searching for the UEFI guilds). Here, all the interaction with `rizin` is done through `rz-pipe`'s Python module.
...
Throwing in a bit more context, SVD files are basically XML files. Still, it wasn't so easy to parse them, at least with [yxml](https://dev.yorhel.nl/yxml) due to its small size and compact design.
...
Actually, the plan on the second phase was to create a plugin inside [rz-lang](https://github.com/rizinorg/rz-lang/)- to create a
